...
def do_get_notes(request):
    ...
    if request.user.is_authenticated() and request.user.is_active:
        ...
        res['notes'] = [];
        for e in arr:
            res['notes'].append({'id': e.id, 'title': e.title, 'content': e.content,});
    else:
        res['status'] = 1
...
def do_update_note(request):
    res = {}
    if request.user.is_authenticated() and request.user.is_active and 'id' in request.POST and 'title' in request.POST and 'content' in request.POST:
        try:
